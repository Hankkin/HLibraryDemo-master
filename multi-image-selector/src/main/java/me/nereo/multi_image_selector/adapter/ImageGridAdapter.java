...
public class ImageGridAdapter extends BaseAdapter {
    ...
    public void setDefaultSelected(ArrayList<Image> resultList) {
        for(Image image1 : resultList){
            Image image = getImageByPath(image1);
            if(image != null){
                mSelectedImages.add(image);
            }
        }
    ...
    private Image getImageByPath(Image image1){
        if(mImages != null && mImages.size()>0){
            for(Image image : mImages){
                if(image.path.equalsIgnoreCase(image1.path)){
                    return image;
                }
            }
        }
...
//                        .centerCrop()
//                        .into(image);
                Glide.with(mContext).load(imageFile).placeholder(R.drawable.default_error).override(mGridWidth,mGridWidth)
